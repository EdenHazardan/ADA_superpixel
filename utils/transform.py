...
def blur_strong(img, p=0.5):
    if random.random() < p:
        kernel_size_y = int(np.floor(np.ceil(0.1 * img.shape[1]) - 0.5 + np.ceil(0.1 * img.shape[1]) % 2))
        kernel_size_x = int(np.floor(np.ceil(0.1 * img.shape[2]) - 0.5 + np.ceil(0.1 * img.shape[2]) % 2))
        kernel_size = (kernel_size_y, kernel_size_x)
        seq = transforms.GaussianBlur(kernel_size=kernel_size, sigma=(0.1, 2.0))
        img = seq(img)
...
def cutout_strong(img, mask, p=0.5, size_min=0.02, size_max=0.4, ratio_1=0.3,
    ...
    if random.random() < p:
        ...
        if pixel_level:
            value = np.random.uniform(value_min, value_max, (img_c, erase_h, erase_w))
        else:
